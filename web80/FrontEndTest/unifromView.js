...
export const UniformView = {
  ...
  renderTable(uniforms) {
    ...
    paginated.forEach((item) => {
      ...
      tr.innerHTML = `
        ...
        <td>${item.uniformQty}</td>
        <td>${item.img ? `<img src="${item.img}" width="40" height="40" style="object-fit: cover; border-radius: 8px; border: 1px solid #ccc;" />` : '-'}</td>
        <td>
          <button class="btn btn-edit" data-id="${item.uniformID}"><i class="fas fa-edit"></i></button>
          <button class="btn btn-delete" data-id="${item.uniformID}"><i class="fas fa-trash"></i></button>
        </td>`;
  ...
  setFormLoading(isLoading = false) {
    ...
    saveBtn.innerHTML = isLoading ? '<i class="fas fa-spinner fa-spin"></i> Saving...' : '<i class="fas fa-save"></i> Save';
  },
